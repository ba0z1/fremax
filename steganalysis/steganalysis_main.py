...
def load_model(Configs, VOCAB_SIZE=None, checkpoint=None):
    ...
    if not Configs.use_plm:
        ...
        elif Configs.model.lower() in ["lstmatt", "att"]:
...
def main(Configs, seed_shift=0):
    ...
    if Configs.use_processor:
        ...
        if Configs.model.lower() in ["gnn"]:
            tokenizer = DistilBertTokenizer(Configs.Tokenizer.model_name_or_path,  do_lower_case=True, padding_side="left")
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--config_path", type=str, default="/data2/steganalysis/configs/att.json")
