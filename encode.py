...
def groupingv2(prob):
    ...
    if prob[0] > mean:
        ...
        #     del indices[idx]
        while prob[-1] < 2*delta:
            idx_list = find_nearest_list(prob, delta*0.05)
            sorted_idx_list = np.sort(idx_list)[::-1]
            for idx in sorted_idx_list:
                groups[0][0] += prob[idx]
                groups[0][1].append(indices[idx])
            ...
            idx_list = find_nearest_list(prob, delta*0.95)
            sorted_idx_list = np.sort(idx_list)[::-1]
            for idx in sorted_idx_list:
                groups[0][0] += prob[idx]
                groups[0][1].append(indices[idx])
...
def FLC_encoder(prob, bit_stream, bit_index, bit, **kwargs):
    ...
    prev = indices[msb_bits2int([int(b) for b in bits_list])].view(1, 1)
    return prev, bit


def AC_encoder(prob, bit_stream, bit_index, cur_interval, top_p = 1.0, precision=52,  **kwargs):
    prob, indices = prob.sort(descending=True)
    ...
    # prob = prob[cum_prob <= top_p]
    if len(prob[cum_prob <= top_p]) == 0:
        prob = prob[0:1]
    else:
        prob = prob[cum_prob <= top_p]
    indices = indices[0:len(prob)]
    ## top p end
    ...
    cum_probs = cum_probs + cur_int_range - cum_probs[-1]  # 分布函数加到和区间数相等，区间数表示的分布函数

    cum_probs = cum_probs + cur_interval[0]  # 分布函数的第一项从左区间开始
...
def ADG_V3_encoder(prob, bit_stream, bit_index, epsilon,  **kwargs):
    mean = 0.5
    # ori_prob = prob
    prob, indices = prob.sort(descending=True)
    # prob_sum = prob.sum()
    ...
    # if abs(groups[0][0]*acc_prob_sum - ori_prob[indices[groups[0][1]]].sum()) > 1e-6:
    #     print()
    while (abs(groups[0][0] - mean) <= epsilon * (2**bit_tmp)) and abs(groups[0][0] - mean)<mean:
        # select groups
        bit = bit_stream[bit_index+bit_tmp]
        if bit_stream[bit_index+bit_tmp] == '0':
            prob = prob[groups[0][1]]
    ...
    prob = prob[:2**kwargs["bit"]]
    indices = indices[:2**kwargs['bit']]
    prob_dict = {i: float(p) for i, p in enumerate(prob)}
    hf = HuffmanCoding()
    hf.make_heap(prob_dict)
    hf.merge_nodes()
    hf.make_codes()
    for hf_code in hf.reverse_mapping.keys():
        if hf_code == bit_stream[bit_index+bit_tmp:bit_index + bit_tmp + len(hf_code)]:
            prev = indices[hf.reverse_mapping[hf_code]].view(1, 1)
            num_bits_encoded = bit_tmp + len(hf_code)
            return prev, num_bits_encoded
...
def ADG_V4_encoder(prob, bit_stream, bit_index, epsilon,  precision=52, **kwargs):
    mean = 0.5
    # ori_prob = prob
    prob, indices = prob.sort(descending=True)
    # prob_sum = prob.sum()
    ...
    #     print()
    # while (abs(groups[0][0] - mean) <= epsilon * (2**bit_tmp)) and abs(groups[0][0] - mean)<mean:
    while (abs(groups[0][0] - mean) <= epsilon) and abs(groups[0][0] - mean) < mean:
        # select groups
        bit = bit_stream[bit_index+bit_tmp]
        if bit_stream[bit_index+bit_tmp] == '0':
            prob = prob[groups[0][1]]
    ...
    cur_interval, prev, num_bits_encoded = AC_encoder(prob, bit_stream, bit_index+bit_tmp, cur_interval, precision)
    return indices[prev].view(1, 1), bit_tmp+num_bits_encoded
...
def PLAIN_encoder(prob, topk=50000, topp=0.99, do_sample=True, **kwargs):
    topk = int(topk)
    topp = float(topp)
    do_sample = True if isinstance(do_sample, bool) or do_sample.lower() == "true" else False
...
def encoder(alg, prob, bit_stream, bit_index, cur_interval=None, top_p = 1, **kwargs):
    if alg.lower() == "plain":
    ...
    if alg.lower() == "ac":
        return AC_encoder(prob, bit_stream, bit_index, cur_interval, top_p, **kwargs)
    if alg.lower() == "adg":
        return ADG_encoder(prob, bit_stream, bit_index, **kwargs)
    if alg.lower() == "adgv2":
        return ADG_V2_encoder(prob, bit_stream, bit_index, **kwargs)
    if alg.lower() == "adgandhc":
        return ADG_V3_encoder(prob, bit_stream, bit_index, **kwargs)
    if alg.lower() == "adgandac":
        return ADG_V4_encoder(prob, bit_stream, bit_index, **kwargs)
